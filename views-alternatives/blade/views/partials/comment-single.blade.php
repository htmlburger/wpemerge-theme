...
<li @php comment_class() @endphp id="li-comment-@php comment_ID() @endphp">
	<div id="comment-@php comment_ID() @endphp" class="comment">
		<div class="comment__author vcard">
			...
			<span class="comment__says">{{ __( 'says:', 'myapp' ) }}</span>
		</div>

		@if ('0' === $comment->comment_approved)
			<em class="comment__moderation-notice">{{ __( 'Your comment is awaiting moderation.', 'myapp' ) }}</em><br />
		@endif

		<div class="comment__meta">
			<a href="{{ esc_url( get_comment_link( $comment->comment_ID ) ) }}">
				{{-- translators: comment date and time --}}
				{{ sprintf( __( '%1$s at %2$s', 'myapp' ), get_comment_date(), get_comment_time() ) }}
			</a>

			@php edit_comment_link( esc_html__( '(Edit)', 'myapp' ), '  ', '' ) @endphp
		</div>
