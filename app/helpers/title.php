...
function mytheme_get_title() {
	...
	if ( is_home() || ( is_single() && get_post_type() === 'post' ) ) {
		...
		$title = sprintf( __( 'Search Results for: %s', 'mytheme' ), get_search_query() );
	} elseif ( is_category() ) {
		/* translators: category post listing page title */
		$title = sprintf( __( 'Category Archives: %s', 'mytheme' ), single_cat_title( '', false ) );
	} elseif ( is_tag() ) {
		/* translators: tag post listing page title */
		$title = sprintf( __( 'Tag Archives: %s', 'mytheme' ), single_tag_title( '', false ) );
	} elseif ( is_day() ) {
		/* translators: day archive post listing page title */
		$title = sprintf( __( 'Daily Archives: %s', 'mytheme' ), get_the_time( 'F jS, Y' ) );
	} elseif ( is_month() ) {
		/* translators: month archive post listing page title */
		$title = sprintf( __( 'Monthly Archives: %s', 'mytheme' ), get_the_time( 'F, Y' ) );
	} elseif ( is_year() ) {
		/* translators: year archive post listing page title */
		$title = sprintf( __( 'Yearly Archives: %s', 'mytheme' ), get_the_time( 'Y' ) );
	} elseif ( is_author() ) {
		/* translators: author archive post listing page title */
		$title = sprintf( __( 'Posts by %s', 'mytheme' ), get_the_author() );
	} elseif ( is_archive() ) {
		$title = __( 'Archives', 'mytheme' );
	} elseif ( is_404() ) {
		$title = __( 'Error 404 - Not Found', 'mytheme' );
	} elseif ( is_page() ) {
		$title = get_the_title();
	}
...
function mytheme_the_title( $before = '', $after = '' ) {
	 ...
	 */
	$after = apply_filters( 'mytheme_get_title_after', $after );

	/**
	 * Get the current main title
	 */
	$title = mytheme_get_title();
	 ...
	 * @param string $title The current main title
	 */
	$title = apply_filters( 'mytheme_the_title', $title );
